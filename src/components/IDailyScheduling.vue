    ...
    <div class="scroll_block">
      <div class="IDailyScheduling_header flex_between">
        ...
        <div class="center flex_center">
          ...
          <div class="selMonth flex_center">
            <div class="select_box">
              <a-select v-model="select_year" @change="changeCurrentTime(e)">
                <a-select-option
                  v-for="(item, index) in getYearsList()"
                  :key="index"
                  :value="item.value"
                >
                  {{ item.label }}
            ...
            <div class="select_box">
              <a-select v-model="select_month" @change="changeCurrentTime(e)">
                <a-select-option
                  v-for="(item, index) in getMonthsList()"
                  :key="index"
                  :value="item.value"
                >
                  {{ item.label }}
        ...
        <div class="right flex_end">
          ...
          <a-upload
            v-if="queryObject.showImportFile == '1'"
            name="file"
            :multiple="false"
            :showUploadList="false"
            :customRequest="doImport"
            accept=".xls,.xlsx"
          >
            <div class="button_box scheduling" style="width: 80px">导入</div>
          </a-upload>
          <div
            v-if="queryObject.showTemplateDownload == '1'"
            class="button_box export"
          ...
          <div
            v-if="queryObject.showHolidayExport == '1'"
            class="button_box holidy_export"
            @click="doExportHoliday()"
          >
            节假日导出
          </div>
          <div
            v-if="queryObject.showReset == '1'"
            class="button_box reset"
            @click="reset()"
          >
            重置
          </div>
      ...
      <div class="table">
        ...
        <div class="table_body">
          ...
          >
            <div
              v-for="(item1, index1) in table_header"
              :key="index1"
              class="cell"
              :class="getCellClassName(item[item1.value])"
            >
              <div class="cell_header flex_between">
                ...
                <div class="right flex_end">
                  ...
                  <span class="solar_calendar">{{
                    getDateValue(item[item1.value].date)
                  }}</span>
                </div>
              </div>
              <div class="cell_body">
                <template
                  v-if="
                    item[item1.value].leaderDuty &&
                    item[item1.value].leaderDuty.length
                  "
                >
                  <div
                    class="leader_duty duty_list flex_between"
                    :class="getDutyListClassName(item[item1.value].leaderDuty)"
                  >
                    <div class="left flex_start">
                      <div
                        v-for="(item3, index3) in item[item1.value].leaderDuty"
                        :key="index3"
                    ...
                    <div class="right">
                      <img
                        v-if="item[item1.value].holidayType == 2"
                        :src="
                          getAssetsImg(
                            propData.leaderAllIcon ? 'quantian' : 'zhiban'
                          )
                        "
                        alt=""
                      />
                      <img
                        ...
                        :src="
                          getAssetsImg(
                            propData.leaderAllIcon ? 'quantian' : 'leader'
                          )
                        "
                        alt=""
                      />
                ...
                <template
                  v-if="
                    ...
                    item[item1.value].dayDuty.length
                  "
                >
                  <div
                    class="day_duty duty_list flex_between"
                    :class="getDutyListClassName(item[item1.value].dayDuty)"
                  >
                    <div class="left flex_start">
                      <div
                        v-for="(item3, index3) in item[item1.value].dayDuty"
                        :key="index3"
                        class="person_list flex_start"
                        @click="jumpToFormPage(item[item1.value], item3)"
                      >
                ...
                <template
                  v-if="
                    ...
                    item[item1.value].nightDuty.length
                  "
                >
                  <div
                    class="night_duty duty_list flex_between"
                    :class="getDutyListClassName(item[item1.value].nightDuty)"
                  >
                    <div class="left flex_start">
                      <div
                        v-for="(item3, index3) in item[item1.value].nightDuty"
                        :key="index3"
                        class="person_list flex_start"
                        @click="jumpToFormPage(item[item1.value], item3)"
                      >
                    ...
                    <div class="right">
                      <img
                        :src="
                          getAssetsImg(
                            propData.leaderAllIcon ? 'night' : 'nightduty'
                          )
                        "
                        alt=""
                      />
      ...
      <div class="timePicker">
        ...
        <a-locale-provider :locale="zhCN">
          <a-date-picker
            v-model="startTime"
            placeholder="请选择开始日期"
            @change="headerStartDatePickerChange"
          ></a-date-picker>
        </a-locale-provider>
      </div>
      <div class="timePicker">
        ...
        <a-locale-provider :locale="zhCN">
          <a-date-picker
            v-model="endTime"
            placeholder="请选择截至日期"
            @change="headerEndDatePickerChange"
          ></a-date-picker>
        </a-locale-provider>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    doImport(info) {
      ...
      IDM.http
        .upload(`ctrl/newSchedule/importExcleRyap`, info.file)
        .then((res) => {
          window.layer.close(loading);
          if (res.data.code == "200") {
            that.getInitData();
    ...
    doDownloadTemplate() {
      IDM.http
        .get(
          `ctrl/newSchedule/templateDownload`,
          {
      ...
      IDM.layer.open({
        ...
        yes: function (index, layero) {
          ...
          if (startTime > endTime) {
            IDM.layer.msg("开始时间不能大于结束时间！");
            return;
          }
          // 格式化并拼接
          const formattedStart = formatDateToYYYYMMDD(startTime);
          const formattedEnd = formatDateToYYYYMMDD(endTime);
    ...
    doExportHoliday() {
      IDM.layer.open({
        ...
        content: IDM.url.getWebPath(
          `ctrl/dutyScheduleCtrl/holiday/select?year=${this.select_year}`
        ),
        area: ["400px", "300px"],
      });
    },
    ...
    schedulingApi() {
      let that = this;
      IDM.http
        .get("ctrl/dutyScheduleCtrl/judgeIsScheduled", {
          year: this.select_year,
          month: this.select_month,
        })
        .then((res) => {
              ...
              this.$confirm({
                ...
                onOk() {
                  let url = IDM.url.getWebPath(
                    `/ctrl/dutyScheduleCtrl/dailyScheduling?year=${that.select_year}&month=${that.select_month}`
                  );
                  window.open(url);
                },
                onCancel() {},
    ...
    schedulingReset() {
      IDM.http
        .get("/ctrl/dutyScheduleCtrl/judgeReset", {
          year: this.select_year,
          month: this.select_month,
        })
        .then((res) => {
    ...
    schedulingResetApi() {
      IDM.http
        .get("/ctrl/dutyScheduleCtrl/resetDuty", {
          year: this.select_year,
          month: this.select_month,
        })
        .then((res) => {
    ...
    getInitData() {
      ...
      IDM.http
        .get(this.propData.queryUrl || "/ctrl/newSchedule/GetFgwSchedule", {
          year: this.select_year,
          month: this.select_month,
    ...
    //上下月切换
    toggleMonth(type) {
      let cMonth = Number(this.select_month);
      let newYear = this.select_year;
      if (type === "prev") {
        cMonth = cMonth - 1;
        if (cMonth < 1) {
          cMonth = 12;
      ...
      this.select_month = cMonth.toString();
      this.select_year = newYear;
      this.getInitData();
    },
    /**
     * 主题颜色
    ...
    convertAttrToStyleObject() {
      ...
      if (
        ...
        window.IDM.setStyleToPageHead(this.moduleObject.id + " .person_list", {
          cursor: "pointer",
        });
      }
  ...
  .table {
    ...
    .table_body {
      .row {
        ...
        .isRelax {
          background: rgba(253, 227, 228, 0.6);
        }
        .cell {
          width: 14.2857%;
          flex-grow: 0;
