<template>
  <div idm-ctrl="idm_module" :id="moduleObject.id" :idm-ctrl-id="moduleObject.id" class="idm-leader-agenda-outer"> 
    <div class="scroll_block">
      <LeaderActiveHeader :ref="moduleObject.id" :moduleObject="moduleObject" :scheduleType="scheduleType" :viewModel="viewModel" :publickStatus="publickStatus" 
      ...
      <template>
        <LeaderActiveEdit :form_data="form_data" :data_list_table="data_list_table"></LeaderActiveEdit>
      </template>
    </div>
  </div>
</template>
...
<script>
const mock = []
import LeaderActiveHeader from "@/workSchedulingComponents/LeaderActiveHeader.vue"
import LeaderActiveEdit from "@/workSchedulingComponents/LeaderActiveEdit.vue"
export default {
  name: 'IWorkSchedulingQuzheng',
  components:{
    LeaderActiveHeader,
    LeaderActiveEdit
  },
  ...
  methods:{
    getRecord() {
      ...
      let url = `/ctrl/list/250609112151Y04JIvWJ5ZTZ2sSanlp?moduleId=250609112150UCGCl6PRFnXLomxu0Ed&sDate=${startDate}&eDate=${endDate}`;
      IDM.layer.open({
        area: ['85vw','90vh'],
        type: 2, 
        content: IDM.url.getWebPath(url)
      }); 
    },
    exportData() {
      ...
      let startDate = days_arr[0];
      let endDate = days_arr[days_arr.length - 1];
      let fileName = `区政处一周工作安排（${startDate}-${endDate}）.xls`;
      IDM.http.post('/ctrl/qzcOneWeekPlan/export',{
        sDate: startDate,
        eDate: endDate,
      },{
        responseType: "blob",
    ...
    save() {
      ...
      IDM.http.post('/ctrl/qzcOneWeekPlan/save',JSON.stringify(this.data_list_table),{
        headers: {
    ...
    getWeekInitData() {
      ...
      let startDate = days_arr[0];
      let endDate = days_arr[days_arr.length - 1];
      IDM.http.get('/ctrl/qzcOneWeekPlan/get',{
        sDate: startDate,
        eDate: endDate,
      }).then((res) => {
